<?php
    ...
    $payment_voucher_result = $crud->common_update("journal_vouchers", $payment_voucher, ['id' => $voucher_id]);
   $crud->common_delete("journal_voucher_details", ['journal_voucher_id' => $voucher_id]);
   $crud->common_delete("ledger", ['journal_voucher_id' => $voucher_id]);
    ...
    foreach ($_POST['account_head_id'] as $index => $account_head_id) {
        ...
        $payment_voucher_detail_result = $crud->common_insert("journal_voucher_details", $details_data);
        $ledger_data = [
            'journal_voucher_id' => $voucher_id,
            'account_head_id' => $account_head_id,
            'dr' => $_POST['dr'][$index] ?? 0,
