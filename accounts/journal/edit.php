...
<!-- Sidebar End -->
 ...
 <?php
    ...
    $voucherData = [];
    if ($voucher_id) {
        $voucherResult = $crud->common_query("SELECT * FROM journal_vouchers WHERE id = $voucher_id");
        if ($voucherResult['status']) {
            $voucherData = $voucherResult['data'][0];
        }
        $voucherDetailsResult = $crud->common_query("SELECT * FROM journal_voucher_details WHERE journal_voucher_id = $voucher_id");
