...
if ($_POST) {
    ...
    if ($result['status']) {
        // rebuild sale details and stock, then recreate accounting entries for this sale
        ...
        if ($result['status'] && $error == 0) {
            ...
            $journal_voucher_id = add_journal_voucher($crud, $id, [
                ['account_id' => $receivable_acc, 'dr' => $receivable_amount, 'cr' => 0, 'remarks' => "Sale #$id - Receivable"],
                ['account_id' => $revenue_acc, 'dr' => 0, 'cr' => $revenue_amount, 'remarks' => "Sale #$id - Revenue"],
                ['account_id' => $vat_output_acc, 'dr' => 0, 'cr' => $tax, 'remarks' => "Sale #$id - VAT Output"]
            ], $receivable_amount, "Sale #$id", $_POST['sale_date']);
